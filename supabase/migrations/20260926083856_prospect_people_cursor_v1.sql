...
-- Compare the additive reader with the established OFFSET reader on real data.
-- The checks are conditional so an empty/new installation still migrates.
do $assert_pages$
declare
...
begin
  ...
  if jsonb_array_length(v_first) = 50 then
    ...
    if (select array_agg(item->>'id') from jsonb_array_elements(v_cursor) item)
      ...
      raise exception 'global People cursor page differs from OFFSET page';
    end if;
  end if;
  ...
  group by client_id
  having count(*) between 51 and 50000
  order by count(*) desc
  limit 1;
  if v_client is not null then
    select result_rows into v_first
    from public.search_prospect_workspace_v12(
      '', '[]'::jsonb, 'created_at', 'desc', 50, 0, v_client, '{}'::jsonb, false, v_versions);
    if jsonb_array_length(v_first) = 50 then
      v_boundary := v_first->49;
      select result_rows into v_cursor
      from public.search_prospect_workspace_cursor_v1(
      ...
      select result_rows into v_offset
      from public.search_prospect_workspace_v12(
        '', '[]'::jsonb, 'created_at', 'desc', 50, 50, v_client, '{}'::jsonb, false, v_versions);
      if (select array_agg(item->>'id') from jsonb_array_elements(v_cursor) item)
         is distinct from
         (select array_agg(item->>'id') from jsonb_array_elements(v_offset) item) then
        raise exception 'client People cursor page differs from OFFSET page for %', v_client;
      end if;
    end if;
  end if;

  select lm.list_id into v_list
  ...
  if v_list is not null then
    v_filters := jsonb_build_array(jsonb_build_object(
      'field', '__list_ids', 'operator', 'contains', 'values', jsonb_build_array(v_list)));
    select result_rows into v_first
    from public.search_prospect_workspace_v12(
      '', v_filters, 'created_at', 'desc', 50, 0, null, '{}'::jsonb, false, v_versions);
    if jsonb_array_length(v_first) = 50 then
      v_boundary := v_first->49;
      select result_rows into v_cursor
      from public.search_prospect_workspace_cursor_v1(
      ...
      select result_rows into v_offset
      from public.search_prospect_workspace_v12(
        '', v_filters, 'created_at', 'desc', 50, 50, null, '{}'::jsonb, false, v_versions);
      if (select array_agg(item->>'id') from jsonb_array_elements(v_cursor) item)
         is distinct from
         (select array_agg(item->>'id') from jsonb_array_elements(v_offset) item) then
        raise exception 'list-filtered People cursor page differs from OFFSET page for %', v_list;
      end if;
    end if;
  end if;
end
