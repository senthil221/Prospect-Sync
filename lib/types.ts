...
export type ClientRecord = { id: string; name: string; list_count: number; prospect_count: number; cooldown_days?: number };
...
export type Company = { id: string; name: string; domain: string; prospect_count: number; client_count: number; created_at: string };
export type ImportRecord = { id: string; file_name: string; data_source: string; client_name: string; list_name: string; processed_rows: number; unique_added: number; duplicates_linked: number; status: string; created_at: string };
export type DeleteKind = "import" | "list" | "client";
export type DeleteRequest = { kind: DeleteKind; id: string; name: string; context: string };
export type FileAudit = { headers: string[]; rows: number; populatedCells: number; invalidRows: number };
...
export type ImportResumeDetail = { id: string; kind: "prospects" | "companies"; listId: string | null; fileName: string; dataSource: string; status: string; committedRowOffset: number; totalRows: number | null; headers: string[]; fieldMap: Record<string, string>; headerSignature: string };
