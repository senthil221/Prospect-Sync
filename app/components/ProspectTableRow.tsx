...
function ProspectTableRow({ prospect, visibleDefinitions, selected, includeClient, canDeleteMaster, onSelect, onToggleSelected, onRemoveFromClient, onDelete }: { prospect: Prospect; visibleDefinitions: ColumnDefinition[]; selected: boolean; includeClient: boolean; canDeleteMaster: boolean; onSelect: (prospect: Prospect) => void; onToggleSelected: (id: string) => void; onRemoveFromClient?: (prospect: Prospect) => Promise<void>; onDelete: (id: string) => void }) {
  return <tr className={selected ? "selected" : ""} onClick={() => onSelect(prospect)}>
