...
function ClientCompanyDatabase({ client, peopleScope, onClearPeopleScope, onSeePeople, onImport }: { client: ClientRecord; peopleScope: PeopleScope | null; onClearPeopleScope: () => void; onSeePeople: (scope: CompanyScope) => void; onImport: () => void }) {
  ...
  const [refresh, setRefresh] = useState(0);
  // Default is busiest-first. A company can sit at zero prospects and still
  // belong on the client's ICP list (20260916180000), so ascending is the only
  // way to reach those rows short of paging to the end.
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!current) return;
        const data = await fetchCompanies<{ companies: Company[]; total: number; covered: number; prospectTotal: number; pageSize: number }>({ search: debouncedSearch, clientId: client.id, page, encodedFilters: requestFilters, peopleScope, sortAscending }, { signal: controller.signal });
