...
export default function ProspectTable({ prospects, total, totalEstimated = false, totalCapped = false, scopeCapped = false, fields, filters, page, clients, search = "", sort, direction, clientId = "", active = true, companyScope = null, onClearCompanyScope, onClearSearch, onSeeCompanies, onRemoveFromClient, onSortChange, onFiltersChange, onPageChange, onSelect, onImport, onRefresh, allowEntityPivot = true, lockedCompanyScope = false }: { prospects: Prospect[]; total: number; totalEstimated?: boolean; totalCapped?: boolean; scopeCapped?: boolean; fields: string[]; filters: ProspectFilter[]; page: number; clients: ClientRecord[]; search?: string; sort: string; direction: "asc" | "desc"; clientId?: string; active?: boolean; companyScope?: CompanyScope | null; onClearCompanyScope?: () => void; onClearSearch?: () => void; onSeeCompanies: (scope: PeopleScope) => void; onRemoveFromClient?: (prospect: Prospect) => Promise<void>; onSortChange: (sort: string, direction: "asc" | "desc") => void; onFiltersChange: (filters: ProspectFilter[]) => void; onPageChange: (page: number) => void; onSelect: (row: Prospect) => void; onImport: () => void; onRefresh: () => void; allowEntityPivot?: boolean; lockedCompanyScope?: boolean }) {
  ...
  // Removing the selection from THIS client. Deliberately a different word, a
  // different button and a different dialog from Delete: one unlinks and is
  // undone by pushing again, the other destroys the master record for every
  // client at once. The two must never be one control with a mode.
  const [removeRequest, setRemoveRequest] = useState<{ count: number } | null>(null);
  const [filtersOpen, setFiltersOpen] = useState(true);
  const clientCooldownDays = clientId ? clients.find((client) => client.id === clientId)?.cooldown_days ?? 90 : 90;
  ...
  // The fixed eight. Uploaded custom keys are no longer offered, so two exports
  // of the same search produce the same columns.
  const exportFieldCatalog = prospectExportPickerFields;
  ...
  function openExportDialog(scope: "all_matching" | "selected") {
    ...
    // Always the full fixed set. This used to seed from whichever table columns
  ...
  // Contact history only. "Add tag" used to live here too: a window.prompt that
  // created an agency-wide tag (prospect_tags.client_id null) belonging to no
  // client. That was a second tag vocabulary running alongside the client ICPs,
  // and the two could not be told apart in the grid, so it is retired - a tag
  // is now something an ICP owns. The existing agency-wide tags and everything
  // carrying them are untouched and still filterable; there is simply no longer
  // a way to make another one.
  async function bulkAction(action: "mark_contacted") {
    if (!selectedCount) return;
    if (selectionMode === "all_matching") { setNotice("Export supports database-wide selection. Select individual rows for contact updates."); return; }
    if (!bulkClientId) { setNotice("Choose a client before marking prospects as contacted."); return; }
    setBulkBusy(true); setNotice("");
    try {
      const result = await api<{ updated: number }>("/api/operations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, prospectIds: [...selectedIds], clientId: bulkClientId }) });
  ...
  async function clientAction(action: "push" | "set_date_contacted" | "set_lead" | "clear_lead" | "remove", targetClientId: string, dateContacted?: string | null) {
    ...
    try {
      ...
      if (action === "push") {
        ...
        // Says what survived, not just what went. The whole contract of this
        // action is that the master record is untouched, and a bare "removed
        // 400" is exactly the sentence that makes somebody think otherwise.
        const removed = Number(result.removed ?? 0);
        setNotice(`Removed ${formatNumber(removed)} ${removed === 1 ? "prospect" : "prospects"} from this client. The People database records are unchanged.`);
  ...
  // Named in a dialog rather than a window.prompt. The prompt froze the tab,
  // could not say what was being saved, had none of the focus contract every
  // other dialog here keeps, and looked nothing like the product - the same
  // reasons the removal confirmation stopped using window.confirm.
  async function saveCurrentView(name: string) {
    const trimmed = name.trim();
    if (!trimmed) return;
    setSavingView(true);
    try {
      const data = await api<{ view: SavedView }>("/api/saved-views", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: trimmed, definition: { filters, columns: visibleColumns, sort, direction } }) });
      setSavedViews((current) => [data.view, ...current]); setNotice(`Saved view “${trimmed}”.`);
  ...
  return <section className="people-workspace">
      ...
      <article className="panel results-panel">
        <div className="results-toolbar">
          ...
          <div className="workspace-actions">
            <MenuButton label="People / company" icon="rows" panelLabel="Maximum people per company" count={maxPeoplePerCompany || undefined}>
              <label className="ds-menu-field"><span>Maximum people per company</span><input className="company-cap-input" aria-label="Max people per company" type="number" min="1" max="1000" value={maxPeoplePerCompany || ""} placeholder="No limit" onKeyDown={(event) => { if (event.key === "ArrowDown" || event.key === "ArrowUp") event.stopPropagation(); }} onChange={(event) => setMaxPeoplePerCompany(event.target.value === "" ? 0 : Number(event.target.value))}/></label>
              <button type="button" className="ds-menu-reset" onClick={() => setMaxPeoplePerCompany(0)}>No limit</button>
            </MenuButton>
        ...
        {selectedCount ? <div className="bulk-bar"><div className="bulk-selection-summary"><strong>{selectedLabel} selected {selectionMode === "all_matching" ? "across all pages" : "across pages"}</strong>{selectionMode === "explicit" && selectedCount < total ? <button onClick={selectAllMatching}>Select all {displayedTotal}</button> : null}</div><div className="bulk-action-group" aria-label="Selection tools"><button onClick={() => openExportDialog("selected")}><AppIcon name="download" size={14}/> Export selected</button></div>{selectionMode === "explicit" ? <div className="bulk-action-group" aria-label="Contact history"><select aria-label="Client for contact history" value={bulkClientId} onChange={(event) => setBulkClientId(event.target.value)}><option value="">Choose client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select><button disabled={bulkBusy || !bulkClientId} onClick={() => void bulkAction("mark_contacted")}><AppIcon name="check" size={14}/> Mark contacted</button></div> : null}
    ...
    {removeRequest && clientId ? <ConfirmDialog
      ...
      scopeNote="The People database records are preserved, and every other client keeps its own link to these people. Push them back to undo this."
      confirmLabel={`Remove ${selectedLabel}`}
      busy={bulkBusy}
      onCancel={() => setRemoveRequest(null)}
      onConfirm={() => void clientAction("remove", clientId)}
    /> : null}
    {saveViewOpen ? <FormDialog titleId="save-view-title" busy={savingView} onClose={() => { setSaveViewOpen(false); setSaveViewName(""); }}><p className="eyebrow">SAVED VIEW</p><h2 id="save-view-title">Name this view</h2><p>Saves the {effectiveFilters.length ? `${effectiveFilters.length} applied filter${effectiveFilters.length === 1 ? "" : "s"}` : "current filters"}, the visible columns and the sort order, so this search can be reopened as it is now.</p><div className="form-field"><label htmlFor="saved-view-name">View name</label><input id="saved-view-name" data-autofocus maxLength={120} value={saveViewName} placeholder="e.g. UK fintech decision makers" onChange={(event) => setSaveViewName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void saveCurrentView(saveViewName); }}/></div><div className="modal-actions"><button className="secondary" disabled={savingView} onClick={() => { setSaveViewOpen(false); setSaveViewName(""); }}>Cancel</button><button className="primary" disabled={savingView || !saveViewName.trim()} onClick={() => void saveCurrentView(saveViewName)}>{savingView ? "Saving…" : "Save view"}</button></div></FormDialog> : null}
