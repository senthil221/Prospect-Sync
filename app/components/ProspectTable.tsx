...
export default function ProspectTable({ prospects, total, totalEstimated = false, totalCapped = false, scopeCapped = false, fields, filters, page, clients, search = "", sort, direction, clientId = "", active = true, companyScope = null, onClearCompanyScope, onClearSearch, onSeeCompanies, onRemoveFromClient, onSortChange, onFiltersChange, onPageChange, onSelect, onImport, onRefresh }: { prospects: Prospect[]; total: number; totalEstimated?: boolean; totalCapped?: boolean; scopeCapped?: boolean; fields: string[]; filters: ProspectFilter[]; page: number; clients: ClientRecord[]; search?: string; sort: string; direction: "asc" | "desc"; clientId?: string; active?: boolean; companyScope?: CompanyScope | null; onClearCompanyScope?: () => void; onClearSearch?: () => void; onSeeCompanies: (scope: PeopleScope) => void; onRemoveFromClient?: (prospect: Prospect) => Promise<void>; onSortChange: (sort: string, direction: "asc" | "desc") => void; onFiltersChange: (filters: ProspectFilter[]) => void; onPageChange: (page: number) => void; onSelect: (row: Prospect) => void; onImport: () => void; onRefresh: () => void }) {
  ...
  return <section className="people-workspace">
    ...
    {saveViewOpen ? <div className="modal-backdrop" role="presentation"><section className="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="save-view-title"><p className="eyebrow">SAVED VIEW</p><h2 id="save-view-title">Name this view</h2><p>Saves the {effectiveFilters.length ? `${effectiveFilters.length} applied filter${effectiveFilters.length === 1 ? "" : "s"}` : "current filters"}, the visible columns and the sort order, so this search can be reopened as it is now.</p><div className="form-field"><label htmlFor="saved-view-name">View name</label><input id="saved-view-name" data-autofocus maxLength={120} value={saveViewName} placeholder="e.g. UK fintech decision makers" onChange={(event) => setSaveViewName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void saveCurrentView(saveViewName); }}/></div><div className="modal-actions"><button className="secondary" disabled={savingView} onClick={() => { setSaveViewOpen(false); setSaveViewName(""); }}>Cancel</button><button className="primary" disabled={savingView || !saveViewName.trim()} onClick={() => void saveCurrentView(saveViewName)}>{savingView ? "Saving…" : "Save view"}</button></div></section></div> : null}
