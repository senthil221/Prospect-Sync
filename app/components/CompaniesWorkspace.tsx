...
export function CompanyTable({ companies, clients = [], total, totalCapped = false, covered, prospectTotal, page, pageSize, clientId = "", search = "", filters = [], peopleScope = null, onClearPeopleScope, onClearSearch, onSeePeople, onFilters, onPageChange, onImport, onRefresh }: { companies: Company[]; clients?: ClientRecord[]; total: number; totalCapped?: boolean; covered: number; prospectTotal: number; page: number; pageSize: number; clientId?: string; search?: string; filters?: ProspectFilter[]; peopleScope?: PeopleScope | null; onClearPeopleScope?: () => void; onClearSearch?: () => void; onSeePeople: (scope: CompanyScope) => void; onFilters?: (filters: ProspectFilter[]) => void; onPageChange: (page: number) => void; onImport: () => void; onRefresh?: () => void }) {
  ...
  return <section className="companies-workspace">
    ...
    <div className="company-summary"><div className="summary-violet" style={{ "--stagger": 0 } as CSSProperties}><span>Companies in database</span><strong><CountUp value={total} suffix={totalCapped ? "+" : ""}/></strong><small>{totalCapped ? "Counting stopped early to keep this fast" : "Complete company directory"}</small></div><div className="summary-blue" style={{ "--stagger": 1 } as CSSProperties}><span>With prospect coverage</span><strong><CountUp value={covered}/></strong><small>{total ? `${Math.round((covered / total) * 100)}% of companies` : "No companies yet"}</small></div><div className="summary-green" style={{ "--stagger": 2 } as CSSProperties}><span>Total linked prospects</span><strong><CountUp value={prospectTotal}/></strong><small>Across all matching companies</small></div><p><AppIcon name="quality" size={17}/><span>Matched by normalized domain first, then company name.</span></p></div>
