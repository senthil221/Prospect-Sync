...
export function CompanyTable({ companies, clients = [], total, totalCapped = false, covered, prospectTotal, page, pageSize, clientId = "", search = "", filters = [], peopleScope = null, onClearPeopleScope, onClearSearch, onSeePeople, onFilters, onPageChange, onImport, onRefresh, allowEntityPivot = true }: { companies: Company[]; clients?: ClientRecord[]; total: number; totalCapped?: boolean; covered: number; prospectTotal: number; page: number; pageSize: number; clientId?: string; search?: string; filters?: ProspectFilter[]; peopleScope?: PeopleScope | null; onClearPeopleScope?: () => void; onClearSearch?: () => void; onSeePeople: (scope: CompanyScope) => void; onFilters?: (filters: ProspectFilter[]) => void; onPageChange: (page: number) => void; onImport: () => void; onRefresh?: () => void; allowEntityPivot?: boolean }) {
  ...
  async function pushCompaniesToClient() {
    if (!pushClientId || !selectedCount) return;
    setPushing(true); setCompanyError(""); setCompanyNotice("");
    try {
      const intent = intentKey({ action: "push-companies", target: pushClientId, selectionMode, ids: selectionMode === "all_matching" ? [] : [...selectedIds], extra: { sourceClientId: clientId || null, search, filters, excluded: [...excludedIds] } });
      ...
      const response = await api<{ result: { selected?: number; added?: number; alreadyPresent?: number; blocked?: number } }>(`/api/clients/${encodeURIComponent(pushClientId)}/companies`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "push", requestId, sourceClientId: clientId || undefined, ...selection }),
      });
      settleIntent(intent);
