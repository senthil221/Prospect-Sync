...
export async function POST(request: Request) {
  ...
  const { importId, listId, headers, rows } = await request.json() as { importId?: string; listId?: string; headers?: string[]; rows?: string[][] };
  ...
  const mapped = rows.map((values) => {
    ...
    return { ...prospect, companyId, normalizedCompanyName: normalizeText(prospect.companyName) };
  });
  const { data, error } = await createAdminClient().rpc("import_prospect_batch", {
