...
function prospectApiPath({ search = "", page = 1, sort = "created_at", direction = "desc", filters = "[]", clientId = "", includeFields = true, companyScope = null, withTotal = page === 1 }: { search?: string; page?: number; sort?: string; direction?: "asc" | "desc"; filters?: string; clientId?: string; includeFields?: boolean; companyScope?: CompanyScope | null; withTotal?: boolean }) {
...
export default function DashboardApp({ currentUserEmail }: { currentUserEmail: string }) {
  ...
  const [prospectTotalEstimated, setProspectTotalEstimated] = useState(false);
        ...
        if (section === "prospects") {
          ...
          if (active) {
            ...
            if (data.total !== null) {
              ...
              prospectTotalCache.current.set(prospectCountKey, cachedTotal);
              setProspectTotal(cachedTotal.total); setProspectTotalEstimated(cachedTotal.estimated);
            } else {
              const cachedTotal = prospectTotalCache.current.get(prospectCountKey);
              if (cachedTotal) { setProspectTotal(cachedTotal.total); setProspectTotalEstimated(cachedTotal.estimated); }
            }
            if (data.fields?.length) { prospectFieldsLoaded.current = true; setProspectFields(data.fields); }
          }
        }
    ...
    <div className="app-shell">
      ...
      <main id="main-content">
        ...
        <section className="content" aria-busy={loading || workspaceLoading}>
          ...
          {!loading && section === "prospects" && <ProspectTable prospects={prospects} total={prospectTotal} totalEstimated={prospectTotalEstimated} fields={prospectFields} filters={prospectFilters} page={prospectPage} clients={clients} search={deferredSearch} sort={prospectSort} direction={prospectDirection} companyScope={companyPeopleScope} onClearCompanyScope={() => setCompanyPeopleScope(null)} onSeeCompanies={(scope) => seeCompanies(scope)} onSortChange={(nextSort, nextDirection) => { setProspectSort(nextSort); setProspectDirection(nextDirection); setProspectPage(1); }} onFiltersChange={(next) => { setProspectFilters(next); setProspectPage(1); }} onPageChange={setProspectPage} onSelect={setSelectedProspect} onImport={() => navigate("imports")} onRefresh={() => setProspectRefresh((current) => current + 1)} />}
...
// The People database shows exactly these mandatory fields as columns.
...
const defaultProspectColumns = standardProspectFields.map((field) => field.id);
...
function ProspectTable({ prospects, total, totalEstimated = false, fields, filters, page, clients, search = "", sort, direction, clientId = "", active = true, companyScope = null, onClearCompanyScope, onSeeCompanies, onRemoveFromClient, onSortChange, onFiltersChange, onPageChange, onSelect, onImport, onRefresh }: { prospects: Prospect[]; total: number; totalEstimated?: boolean; fields: string[]; filters: ProspectFilter[]; page: number; clients: ClientRecord[]; search?: string; sort: string; direction: "asc" | "desc"; clientId?: string; active?: boolean; companyScope?: CompanyScope | null; onClearCompanyScope?: () => void; onSeeCompanies: (scope: PeopleScope) => void; onRemoveFromClient?: (prospect: Prospect) => Promise<void>; onSortChange: (sort: string, direction: "asc" | "desc") => void; onFiltersChange: (filters: ProspectFilter[]) => void; onPageChange: (page: number) => void; onSelect: (row: Prospect) => void; onImport: () => void; onRefresh: () => void }) {
  ...
  return <section className="people-workspace">
    ...
    <div className="people-tabs">
      <button className={tab === "records" ? "active" : ""} onClick={() => setTab("records")}>All prospects <span>{displayedTotal}</span></button>
    ...
    {deleteRequest ? <div className="modal-backdrop" role="presentation"><section className="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="prospect-delete-title"><span className="warning-mark">!</span><p className="eyebrow">PERMANENT ACTION</p><h2 id="prospect-delete-title">Delete {formatNumber(deleteRequest.count)} {deleteRequest.count === 1 ? "prospect" : "prospects"}?</h2><p>This permanently removes {deleteRequest.count === 1 ? "this prospect" : "these prospects"} from the People database, including their list links across every client. This cannot be undone. Company records are not affected.</p>{deleteRequest.mode === "all_matching" && !search.trim() && !effectiveFilters.length && !excludedIds.size ? <p className="form-error" role="alert">⚠ No search or filters are applied — this will empty your entire People database.</p> : null}<div className="modal-actions"><button className="secondary" disabled={deletingProspects} onClick={() => setDeleteRequest(null)}>Cancel</button><button className="danger-button solid" disabled={deletingProspects} onClick={() => void confirmDeleteProspects()}>{deletingProspects ? "Deleting…" : `Delete ${formatNumber(deleteRequest.count)}`}</button></div></section></div> : null}
  </section>;
}
...
function CompanyTable({ companies, total, covered, prospectTotal, page, pageSize, clientId = "", search = "", filters = [], peopleScope = null, onClearPeopleScope, onSeePeople, onFilters, onPageChange, onImport, onRefresh }: { companies: Company[]; total: number; covered: number; prospectTotal: number; page: number; pageSize: number; clientId?: string; search?: string; filters?: ProspectFilter[]; peopleScope?: PeopleScope | null; onClearPeopleScope?: () => void; onSeePeople: (scope: CompanyScope) => void; onFilters?: (filters: ProspectFilter[]) => void; onPageChange: (page: number) => void; onImport: () => void; onRefresh?: () => void }) {
  ...
  return <section className="companies-workspace">
    ...
    {onFilters && bulkOpen ? <div className="panel company-bulk-panel"><div className="company-bulk-panel-head"><div><strong>Filter by a list of domains</strong><small>Paste domains — each is normalized and matched against company websites. They stack with your other filters.</small></div><button className="company-bulk-close" aria-label="Close bulk domains" onClick={() => setBulkOpen(false)}>×</button></div><BulkDomainPaste onAdd={(domains) => { onFilters(addDomainsToWebsiteFilter(filters, domains)); onPageChange(1); }} />{domainFilterCount ? <button type="button" className="company-bulk-clear" onClick={() => { onFilters(filters.filter((filter) => !(filter.field === "__website" && (filter.operator === "contains" || filter.operator === "equals")))); onPageChange(1); }}>Clear {domainFilterCount} domain{domainFilterCount === 1 ? "" : "s"}</button> : null}</div> : null}
...
function DeleteConfirmation({ target, busy, onCancel, onConfirm }: { target: DeleteRequest; busy: boolean; onCancel: () => void; onConfirm: () => Promise<void> }) {
